...
const uploadProfile = async (req, res) => {
    try {
        ...
        if (profileDetail && profileDetail.imageUrls && (profileDetail.imageUrls.length < 3)) {
            ...
            res.send({ "isSuccess": true, "message": "Image uploaded successfully" })
        }
        else {
...
const uploadProfileImage = async (req, res) => {
    try {
        ...
        if (profileDetails.imageUrls && profileDetails.imageUrls.length < 3) {
            ...
            return res.status(200).json({ isSuccess: true, message: "Image uploaded successfully" });
        } else {
...
const PUImageMovedToBrokContainer = async (sourceContainer, destContainer, imageName) => {
    try {
        ...
        const sourceContainerClient = blobServiceClient.getContainerClient(sourceContainer.toLowerCase())
        const destContainerClient = blobServiceClient.getContainerClient(destContainer.toLowerCase())

        const sourceBlobClient = sourceContainerClient.getBlobClient(encodedImageName)
        const destBlobClient = destContainerClient.getBlobClient(encodedImageName)
        ...
        // Copy the blob directly using URL (no SAS needed if same storage account)
        ...
        console.error("Blob copy failed:", error.message)
        return false
    }
};
