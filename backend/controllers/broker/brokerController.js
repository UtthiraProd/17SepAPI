...
const {PUImageMovedToBrokContainer} = require('../../azureservice/fileUploadService')
    ...
    try {
        ...
        // let _brokerDetail = null;
           ...
           if (!brokerId) {
                ...
                const broker = await Broker.findOne({userId:user._id})
// console.log("magesh",broker)
        //    else {
        //           _brokerDetail = await Broker.findById({_id:_brokerId});
        //           console.log("magesh",_brokerDetail)
        //     }
      ...
      delete publicUserObj._id; // remove _id

if (
  publicUserObj.horoScope &&
  typeof publicUserObj.horoScope === 'object' &&
  publicUserObj.horoScope !== null
) {
  delete publicUserObj.horoScope.profileId;
}


// ✅ Use publicUserObj to create new profile
const newMarriageProfile = new MarriageProfile({
  ...
  isPublicProfile:true
    ...
    if (
    newMarriageProfile.horoScope &&
    typeof newMarriageProfile.horoScope === 'object'
) {
    newMarriageProfile.horoScope.profileId = newMarriageProfile._id;
}
      await newMarriageProfile.save();
        console.log('bala')

         const sourceContainer = publicUser.container;

         const imageUrls = publicUser.imageUrls || [];
 
        const brokerContainer = broker?.container 

        for (const image of imageUrls) {
    const imageName = image.name;

    const success = await PUImageMovedToBrokContainer(sourceContainer, brokerContainer, imageName)
    if (!success) {
      return
    }
}
...
const getMatchProfile = asyncHandler(async(req,res)=>{
    ...
    try{
        ...
        let _pagesize = 8;
           ...
           const {matchProfile,skip,pagesize, selectedStars
            } = req.body
        ...
        let totalRecords = 0;
           ...
           const cleanSelectedStars = Array.isArray(selectedStars)
      ? selectedStars
      ...
      : [];
        ...
        //    console.log('matchingMarriageProfiles',matchingMarriageProfiles)
            ...
            sex: oppositeSex,
             ...
             console .log('matchedStars',matchedStars)
        ...
        //    console.log('matched profile',matchingMarriageProfiles)
