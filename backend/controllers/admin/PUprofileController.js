...
const getAllPUMarriageProfile = asyncHandler(async(req,res)=>{
  try{
    ...
    const marriageProfile = await PUMarriageProfile.find(query).skip(_skip).limit(_pagesize);

    const _totalRecord = await PUMarriageProfile.find(query).countDocuments()

    const PUProfileImage = await Promise.all(
          marriageProfile.map(async (doc) => {
        try {
            const image = doc.imageUrls|| [];
            let imageName = '';
      ...
      return {
          ...
          console.error(`Error processing image for profile ${doc._id}:`, err);
          return {success: false,  _id: doc._id, imageBase64: null };
        }
      })
      )
...
const AdmindeletePUprofile = asyncHandler(async(req,res)=>{
  try{
      const user = await User.findById(req.user.id)
...
// console.log("sanjay",profileID)
              const deleteprofile = await PUMarriageProfile.findByIdAndUpdate({_id: profileID},{ $set: { status: "Deleted" }})

              console.log("sanjay",deleteprofile)
         if(deleteprofile){
            res.status(201).json({isSuccess:true,message:"Profile deleted successfully"})
        }
        else{
            console.log("Error occur")  
        }
